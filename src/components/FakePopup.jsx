...
export default function FakePopup({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center" onClick={handleClose}>
      <div
        className={`bg-gray-200 border-2 border-gray-600 rounded-lg shadow-2xl max-w-sm md:max-w-md w-full mx-4 ${closing ? 'animate-spin' : ''}`}
        style={{
          transform: clickCount === 1 ? 'translate(20px, -15px) rotate(3deg)' : clickCount === 2 ? 'translate(-30px, 20px) rotate(-5deg)' : '',
      ...
      >
        {/* Title bar */}
        <div className="bg-gradient-to-r from-blue-800 to-blue-600 text-white px-3 md:px-4 py-2 rounded-t-lg flex justify-between items-center text-xs md:text-sm font-bold">
          <span className="truncate mr-2">{popup.icon} {popup.title}</span>
          <button onClick={handleClose} className="hover:bg-red-500 px-2 rounded">✕</button>
        </div>
        {/* Content */}
        <div className="p-4 md:p-6 text-black">
          <div className="flex gap-3 md:gap-4 items-start">
            <span className="text-3xl md:text-5xl">{popup.icon}</span>
            <p className="text-xs md:text-sm whitespace-pre-line">{popup.text}</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-center mt-4 md:mt-6">
            <button onClick={handleClose} className="bg-gray-300 border border-gray-500 px-4 md:px-6 py-1.5 rounded text-xs md:text-sm hover:bg-gray-400 font-bold">
              OK (je souffre)
            </button>
            <button onClick={handleClose} className="bg-gray-300 border border-gray-500 px-4 md:px-6 py-1.5 rounded text-xs md:text-sm hover:bg-gray-400 font-bold">
