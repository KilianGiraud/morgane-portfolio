...
export default function Navbar() {
  return (
    <nav className="bg-black/70 backdrop-blur-sm border-b-4 border-dashed border-cyan-400 sticky top-10 z-40">
      <div className="max-w-5xl mx-auto flex flex-wrap justify-center gap-1 p-2">
        {links.map(({ to, label }) => (
          <NavLink
            key={to}
            to={to}
            ...
            className={({ isActive }) =>
              `px-4 py-2 font-bold text-sm md:text-base rounded-lg transition-all duration-200 
      ...
      {/* Visitor counter */}
      <div className="text-center py-1 text-sm text-cyan-300 border-t border-cyan-800">
        🔥 Visiteur n° <strong className="text-yellow-300">{(Math.floor(Math.random() * 900000) + 100000).toLocaleString('fr-FR')}</strong> 🔥
        <span className="ml-3 text-gray-500">⚠️ Optimisé pour Internet Explorer 6 ⚠️</span>
      </div>
    </nav>
  )
}
